...
#### 随机起名
- **7 种姓名风格**：中文（现代/古典/玄幻）、英文、日文、西幻、虚构
- **丰富的名字库**：数百个精选名字，组合可能性达数千至数万
- **智能交互**："再来一组"功能，快速生成更多选项
- **多种操作**：创建人物文件、复制到剪贴板、插入到光标位置
- 命令：`Noveler: 随机起名`，或通过侧边栏"人物设定"组调用
...
- ✅ 字数统计重大优化（精准标点识别、空格过滤）
- ✅ 章节重命名智能化

### v0.4.0 ✅
- ✅ 敏感词检测功能（三级词库系统）
- ✅ Quick Fix 支持（添加白名单、删除、替换）
- ✅ 自定义词库（黑名单/白名单）
- ✅ 敏感词配置优化（JSONC 格式、自动创建）
