...
写作过程中随手标记疑问和待处理内容，侧边栏统一收束，提醒自己稍后回头解决。

**使用方法**：

1. 在任意 Markdown 文件中，将光标移到需要标记的位置
2. 按 `Ctrl+Shift+/`，输入待办内容，回车确认
3. 该行末尾自动插入 `<!--? 内容 -->` 标记，行内装饰高亮提示

**侧边栏**：

- 「写作待办」节点展示所有标记，按文件分组，显示待处理 / 总计数量
- 点击条目直接跳转到对应文件和行
- 条目右键：标记为已完成 / 重新打开 / 删除
- 根节点右键：筛选显示（全部 / 仅待处理）、批量完成、批量删除已完成

**编辑器内**：

- 待处理行显示橙色左边框
- 已完成行半透明置灰
- 标记行上方显示 CodeLens 快捷操作：`$(bookmark) 写作待办  $(check) 标记已完成  $(trash) 删除`
...
"noveler": {
  "doubtMark": {
    "deleteOnResolve": false  // true = 标记完成时直接删除该行
  }
}
```
...
| `Noveler: 添加写作待办` | 在当前行插入待办标记 | `Ctrl+Shift+/` |
...
### 当前版本 v0.8.6
- 📌 **写作待办**：`Ctrl+Shift+/` 随手标记，侧边栏统一收束管理，支持 CodeLens 快捷操作
- 优化快速设置 UI，配置项按分组展示，交互更一致
- 侧边栏「工具与设置」拆分为「写作工具」和「项目设置」两级分组
- 侧边栏节点图标全面换用 ThemeIcon，随主题明暗自动切换
- 修复 CodeLens 进度图标损坏（显示为两个问号）

### v0.8.5
- 修复分卷模式下章节创建的多个问题（不打开编辑器、targetWords 不读配置、编号重复）
- 修复自���空行持续触发导致文件不断增加换行的问题
- 修复手机预览中单换行不显示段落分隔的问题
- 提取章节创建公共逻辑，消除重复实现

### v0.8.4
- 🎵 打字音效（8 种内置音效 + 自定义音效文件，默认关闭）

### v0.8.0 ~ v0.8.3
- 📱 手机阅读预览（仿真 iPhone 外观，4 种主题，实时同步光标段落）
- 👁 护眼模式（低饱和度暖灰绿主题，项目隔离不影响其他工作区）
- 🚀 性能优化（Trie 树人物高亮、配置加载竞态修复、字数统计缓存）
...
### 后续计划
- 📥 批量导入已有章节文件（txt 批量导入并自动生成 Front Matter）
- 🤖 AI 写作助手集成
- 📈 写作目标跟踪
- 🔄 更丰富的导出格式（EPUB、PDF）
