...
- **写作待办**：`Ctrl+Shift+/` 在当前行插入待办标记，也可右键菜单触发
  - 侧边栏「写作待办」节点按文件分组展示，显示待处理 / 总计数量
  - 点击条目跳转到对应文件行；条目和根节点右键提供完整操作菜单（标记完成、重新打开、删除、筛选、批量操作）
  - 编辑器行内高亮：待处理橙色左边框，已完成半透明灰
  - 标记行上方 CodeLens 快捷操作：标记已完成 / 重新打开 / 删除
  - 支持配置完成时自动删除行（`noveler.doubtMark.deleteOnResolve`）
  - 标记语法为 HTML 注释，不影响正文显示和导出

## [0.8.6] - 2026-06-06

### 新增

- **章节编号模式快速设置**：可在「快速设置」中直接切换全局连续/按卷重置/混合三种模式，无需手动编辑配置文件

### 优化

- **快速设置 UI 重构**：配置项按「写作格式 / 视觉样式 / 分卷设置 / 专注写作」分组展示；toggle 类设置（空行/缩进/护眼）统一改为子选择器交互，消除直接执行的歧义；分卷设置分组仅在分卷模式启用时出现
- **侧边栏「工具与设置」重构**：拆分为「写作工具」（统计仪表板/手机预览/随机起名）和「项目设置」（快速设置/敏感词库/配置文件）两个子分组，移除与快速设置重复的护眼/专注模式入口
- **侧边栏图标全面换用 ThemeIcon**：所有节点图标由嵌入 emoji 改为 VS Code 原生 ThemeIcon，随明暗主题自动切换，视觉更原生；卷状态用 circle-outline/edit/pass 三态区分
- **章节编号默认值**：分卷模式下章节编号默认从 `global`（全局连续）改为 `volume`（按卷重置），避免往非末尾卷插入章节时产生编号冲突

### 修复

- **CodeLens 进度图标显示两个问号**：50%–99% 进度区间的 emoji 字符在源文件中截断损坏，现已修复

## [0.8.5] - 2026-06-05

### 修复

- **分卷模式章节创建**：新建章节后不打开编辑器的问题
- **分卷模式 targetWords**：目标字数始终显示 2500，未读取 novel.jsonc 配置的问题
- **章节编号重复**：分卷模式下快速连续创建章节时编号重复的问题
- **Front Matter 字段顺序**：分卷与扁平模式生成的字段顺序不一致
- **迁移后创建章节报错**：从扁平迁移到分卷模式后，新建章节报"未找到 README.md"错误
- **自动空行持续触发**：VS Code 保存文件时触发换行事件导致持续插入空行
- **手机预览换行**：VS Code 中单个换行在手机预览中不显示段落分隔的问题
- **敏感词服务未启动**：initializeSensitiveWordService 定义后从未调用，导致敏感词功能实际不工作

### 优化

- 提取 `writeAndOpenChapter()` 公共函数，消除章节创建的重复逻辑
- 将硬编码数字（2500、500000、100000、500）统一提取为常量
...
#### 🎵 打字音效
- **8 种内置音效**：机械键盘、打字机、气泡、pop、click 等
- **自定义音效**：支持添加多个本地音效文件（mp3/wav/ogg/m4a），选择时自动试听
- **保存无声**：保存文件时不触发音效，避免干扰
- 默认关闭，不影响普通用户
...
#### 🚀 性能提升
- **人物名高亮**：使用 Trie 树替代正则表达式匹配，时间复杂度从 O(n*m) 降至 O(n)，大幅提升多人物场景下的高亮性能
...
#### 🔧 性能与稳定性
- **ConfigService**：修复配置加载竞态条件，添加加载锁防止并发加载
- **WordCountService**：添加缓存过期机制（5分钟 TTL）和最大缓存数限制（100条），防止内存无限增长
- **VolumeService**：添加缓存机制（30秒 TTL）和文件监听器自动失效，避免频繁扫描

## [0.8.1] - 2026-02-03

### 修复

- **护眼模式**：修复首次启用后无法关闭的问题（现在会正确保存全局主题）
- **手机预览**：修复小屏幕设备上预览面板显示不全的问题（自动缩放适配）
...
#### 📱 手机阅读预览
- **仿真 iPhone 15/16 Pro 外观**：灵动岛、侧边按钮、状态栏等细节还原
...
#### 🎯 光标位置同步
- **段落高亮**：预览区域自动高亮当前编辑的段落
- **智能滚动**：光标移动时，预览自动滚动到对应位置（视窗 1/3 处）
- **底部留白**：200px 底部留白，最后一行不再贴底

#### 🔧 交互优化
- **设置面板保持**：切换主题/字号时设置面板不再关闭
- **点击关闭设置**：点击内容区域自动关闭设置面板
- **边界值反馈**：字号/行距到达边界时按钮变灰提示
- **智能自动滚动**：在底部编辑时自动跟随，中间阅读时保持位置

### 入口
- 侧边栏「工具与设置」→「📱 手机预览」
- 编辑器右键菜单 →「Noveler: 手机阅读预览」
- 命令面板 → `Noveler: 手机阅读预览`
...
#### 👁 护眼模式
- **完整护眼主题**：低饱和度暖灰绿背景，科学设计保护视力
  - 编辑器背景 `#2d3830`，前景 `#e8f0e4`
  - 完整的 UI 颜色配置（侧边栏、标签页、终端等）
  - 语法高亮配色（对话、人物、Markdown 等）
- **一键切换**：侧边栏"工具与设置" → "切换护眼模式"
- **项目隔离**：仅影响当前项目，不影响其他工作区
- **状态同步**：扩展启动时自动同步主题与配置状态
