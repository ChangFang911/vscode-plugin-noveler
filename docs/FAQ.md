...
### Q: 如何开启护眼模式？
**A**: 侧边栏「工具与设置」→「切换护眼模式」，或命令面板输入 `Noveler: 切换护眼模式`。护眼模式使用低饱和度暖灰绿主题，仅影响当前工作区，不影响其他项目。

### Q: 护眼模式关闭后主题没有恢复怎么办？
**A**: 首次启用时插件会保存当前主题到 `novel.jsonc` 的 `eyeCareMode.previousTheme` 字段，关闭时自动恢复。如果字段为空，可以手动在 `novel.jsonc` 中填入原主题名，或直接在 VS Code 设置中切换主题。

### Q: 如何开启打字音效？
**A**: 命令面板输入 `Noveler: 打字音效设置`，或侧边栏「工具与设置」→「打字音效」。支持 8 种内置音效，也可以添加本地 mp3/wav 文件作为自定义音效。保存文件时不会触发音效。

### Q: 手机预览如何打开？
**A**: 侧边栏「工具与设置」→「手机预览」，或编辑器右键菜单 →「Noveler: 手机阅读预览」。预览会实时同步编辑内容，并高亮当前光标所在段落。

### Q: 手机预览中换行和编辑器不一致？
**A**: 预览按单换行分段，VS Code 编辑器里每按一次回车就是一个新段落，不需要加空行。
